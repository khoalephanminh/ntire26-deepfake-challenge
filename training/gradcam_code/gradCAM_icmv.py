...
def visualize_gradcam(model, input_tensors, output_folder, item_idx, use_smooth=0):
    ...
    if model_name == 'MixModel_x3_crop5img':
        layer_model_eff = model.backbone.model_2.efficientnet._conv_head
        layer_model_cvt = model.backbone.model_1.cvt.cvt.encoder.stages[2].layers[19].attention.attention.convolution_projection_value.convolution_projection.convolution
        
        target_layers_eff = [layer_model_eff]
        target_layers_cvt = [layer_model_cvt]
    ...
    input_original = input_tensors["image_original"]
    ...
    # print("shapes=", input_original[0].shape, input_tensor_1.shape, input_tensor_2.shape, input_tensor_3.shape)

    # with GradCAMPlusPlus(model=model.backbone, target_layers=target_layers) as cam:
    with GradCAMPlusPlus(model=grad_model, target_layers=target_layers_eff) as cam:
        # You can also pass aug_smooth=True and eigen_smooth=True, to apply smoothing.
        torch.set_grad_enabled(True)  # required for grad cam
        if use_smooth:
            grayscale_cam = cam(input_tensor=input_tensor_2, targets=targets, aug_smooth=True, eigen_smooth=True)
        else:
            grayscale_cam = cam(input_tensor=input_tensor_2, targets=targets)
        
        B_outer = input_tensor_2.size(0)    # outer batch size
        B_inner = input_tensor_2.size(1)    # inner batch size (5)

        grayscale_cam = grayscale_cam.reshape(B_outer, B_inner, *grayscale_cam.shape[1:])
        ...
        for outer_idx in range(B_outer):
            ...
            # print("grayscale_cam_batch=", grayscale_cam_batch.shape)
            preds_batch = cam.outputs                # shape: (5, num_classes)
            prob = torch.softmax(preds_batch[outer_idx], dim=0)[1]
            prob = f"{prob.item():.6f}"
            ...
            for inner_idx in range(B_inner):
                ...
                output_path = os.path.join(output_folder, 
                                        f'gradcam_outer{outer_idx}_{prob}_inner{inner_idx}_eff.png')
                plt.imsave(output_path, visualization)
                # print(f"Grad-CAM saved: {output_path}")
            
            # numpy_image_original = input_original[outer_idx].astype(np.uint8)
            # output_path_original = os.path.join(output_folder, 
            #                                 f'gradcam_outer{outer_idx}_{prob}_original_eff.png')                             
    ...
    # gradCAM for cvt
    with GradCAMPlusPlus(model=grad_model, target_layers=target_layers_cvt) as cam:
        # You can also pass aug_smooth=True and eigen_smooth=True, to apply smoothing.
        torch.set_grad_enabled(True)  # required for grad cam
        if use_smooth:
            grayscale_cam = cam(input_tensor=input_tensor_1, targets=targets, aug_smooth=True, eigen_smooth=True)
        else:
            grayscale_cam = cam(input_tensor=input_tensor_1, targets=targets)
        
        B_outer = input_tensor_1.size(0)    # outer batch size
        B_inner = input_tensor_1.size(1)    # inner batch size (5)

        grayscale_cam = grayscale_cam.reshape(B_outer, B_inner, *grayscale_cam.shape[1:])

        for outer_idx in range(B_outer):
            # For each sample in outer batch
            inner_batch_tensor = input_tensor_1[outer_idx]   # shape: (5, 3, 380, 380)
            ...
            for inner_idx in range(B_inner):
                ...
                # Save Grad-CAM visualization
                output_path = os.path.join(output_folder, 
                                        f'gradcam_outer{outer_idx}_{prob}_inner{inner_idx}_cvt.png')
                plt.imsave(output_path, visualization)
                # print(f"Grad-CAM saved: {output_path}")
            
            numpy_image_original = input_original[outer_idx].astype(np.uint8)
            output_path_original = os.path.join(output_folder, 
                                            f'gradcam_outer{outer_idx}_{prob}_original_cvt.png')                             
            cv2.imwrite(output_path_original, cv2.cvtColor(numpy_image_original, cv2.COLOR_RGB2BGR))
            print(f"Original image saved: {output_path_original}")
