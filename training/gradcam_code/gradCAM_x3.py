...
def visualize_gradcam(model, input_tensors, output_folder, item_idx, use_smooth=0):
    ...
    if model_name == 'MixModel' or model_name == 'MixModel_x3':
        ...
        # layer_model_3 = model.backbone.model_3.svd_model.vision_model.post_layernorm
        # layer_model_3 = model.backbone.model_3.svd_model.vision_model.embeddings.patch_embedding
        layer_model_3 = model.backbone.model_3.svd_model.vision_model.encoder.layers[-1].mlp.fc2

        # layer_model_3 = model.backbone.model_3.svd_model.vision_model.encoder.layers[-1].layer_norm1
    ...
    input_tensor = input_tensors[1].to(device)
    input_tensor_2 = input_tensors[2].to(device)
    input_tensor_3 = input_tensors[3].to(device)

    # print("shapes=", input_tensor.shape, input_tensor_2.shape, input_tensor_3.shape)

    grad_model.grad_images = [input_tensor, input_tensor_2, input_tensor_3]
    ...
    with GradCAMPlusPlus(model=grad_model, target_layers=target_layers_2) as cam:
        ...
        for i in range(input_tensor_2.size(0)):
            single_image_tensor = input_tensor_2[i].unsqueeze(0)
            rgb_img = single_image_tensor.cpu().numpy().transpose(0, 2, 3, 1)[0]
            rgb_img = (rgb_img - rgb_img.min()) / (rgb_img.max() - rgb_img.min())
            grayscale_cam_single = grayscale_cam[i, :]
            visualization = show_cam_on_image(rgb_img, grayscale_cam_single, use_rgb=True)
    ...
    with GradCAMPlusPlus(model=grad_model, target_layers=target_layers_3) as cam:
        ...
        torch.set_grad_enabled(True)  # required for grad cam
        if use_smooth:
            grayscale_cam = cam(input_tensor=input_tensor_3, targets=targets, aug_smooth=True, eigen_smooth=True)
        else:
            grayscale_cam = cam(input_tensor=input_tensor_3, targets=targets)
        
        for i in range(input_tensor_3.size(0)):
            single_image_tensor = input_tensor_3[i].unsqueeze(0)
            rgb_img = single_image_tensor.cpu().numpy().transpose(0, 2, 3, 1)[0]
            rgb_img = (rgb_img - rgb_img.min()) / (rgb_img.max() - rgb_img.min())
            grayscale_cam_single = grayscale_cam[i, :]
            visualization = show_cam_on_image(rgb_img, grayscale_cam_single, use_rgb=True)
