...
def add_Gaussian_noise(img, noise_level1=2, noise_level2=25, strength=1):
    ...
    rnum = np.random.rand()
    if rnum < 0.70: img += np.random.normal(0, noise_level/255.0, img.shape).astype(np.float32)
    elif rnum < 0.90: img += np.random.normal(0, noise_level/255.0, (*img.shape[:2], 1)).astype(np.float32)
    else:
...
def add_speckle_noise(img, noise_level1=2, noise_level2=25, strength=1):
    noise_level = random.randint(noise_level1, noise_level2) * strength
    rnum = random.random()
    if rnum > 0.6: img += img*np.random.normal(0, noise_level/255.0, img.shape).astype(np.float32)
    elif rnum < 0.4: img += img*np.random.normal(0, noise_level/255.0, (*img.shape[:2], 1)).astype(np.float32)
    else:
        L = noise_level2/255. * strength
        conv = np.dot(np.dot(np.transpose(orth(np.random.rand(3,3))), np.diag(np.random.rand(3))), orth(np.random.rand(3,3)))
...
def add_Poisson_noise(img):
    ...
    if random.random() < 0.5:
        ...
        img_gray = np.clip((np.dot(img[...,:3], [0.299, 0.587, 0.114]) * 255.0).round(), 0, 255) / 255.
        img += (np.random.poisson(img_gray * vals).astype(np.float32) / vals - img_gray)[:, :, np.newaxis]
    return np.clip(img, 0.0, 1.0)
